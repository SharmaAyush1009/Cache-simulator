...
int main() {
    ...
    while (std::getline(configFile, line)) {
        ...
        else if (key == "EnablePrefetch") EnablePrefetch = std::stoi(value);
    }
    configFile.close();
    ...
    if (rwOps.empty()) {
        std::cout << "Trace empty or file not found.\n";
        return 1;
    }
    
    // Build cache levels
    CacheLevel L1(L1CacheSize, L1BlockSize, L1Associativity, L1Latency,WritePolicyL1 == 1 ? WritePolicy::WriteBack : WritePolicy::WriteThrough,
        AllocatePolicyL1 == 1 ? AllocatePolicy::WriteAllocate : AllocatePolicy::NoWriteAllocate);
    
    CacheLevel L2(L2CacheSize, L2BlockSize, L2Associativity, L2Latency,WritePolicyL2 == 1 ? WritePolicy::WriteBack : WritePolicy::WriteThrough,
    ...
    cacheHierarchy.setPrefetch(EnablePrefetch == 1);
